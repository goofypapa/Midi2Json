...
namespace ws_core
{
    ...
    node * parse( const char * p_json )
    {
        ...
        if( err >= 0 )
        {
            err( "format err by " << err << " " << p_json[err - 2] << p_json[err - 1] << p_json[err] << p_json[err + 1] << p_json[err + 2] );
